...
public:
    bool isValidSudoku(vector<vector<char>>& board) {
        unordered_set<char> row[9], col[9], box[9]; //Create 9 unordered_sets for each row, column, and box
        ...
        for(int i = 0; i < 9; i++){
            for(int j = 0; j < 9; j++){
                ...
                if(row[i].count(index) || col[j].count(index) || box[boxIndex].count(index))
                    return false; 
                row[i].insert(index); 
                col[j].insert(index); 
                box[boxIndex].insert(index); 
            }
        }
        return true;
    }
